...
RUN CGO_ENABLED=0 go build -v -a -tags PROJECTNAME -installsuffix PROJECTNAME -ldflags "-X main.version=${VERSION} -X main.revision=${REVISION}" -o /go/bin/PROJECTNAME
